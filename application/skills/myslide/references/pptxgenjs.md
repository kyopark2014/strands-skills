...
// Rounded rectangle — DEFAULT for all card/container shapes
// Use line property for accent colors instead of overlay bars
slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  x: 1, y: 1, w: 3, h: 2, rectRadius: 0.1,
  fill: { color: "161E2D" },
  line: { color: "C91F8A", width: 1.5 }  // accent border
});

// Radius guidelines:
//   Large cards: 0.10-0.12   Inner boxes: 0.06-0.08
//   Tag badges: 0.15 (pill)  Summary bars: 0.08
...
### Gradient Text (SVG Workaround)

PptxGenJS doesn't support gradient fills on text. To create gradient text (e.g., a big
"80%" with an orange-to-pink gradient), render it as an SVG and convert to PNG with sharp.
...
async function gradientText(text, opts = {}) {
  const {
    ...
    fontWeight = "bold",
    colorStart = "#F66C02",   // orange
    colorEnd = "#FF28EF",     // pink
  ...
  const d = dirs[direction] || dirs.horizontal;
...
// Usage:
const heroNum = await gradientText("80%", {
  width: 900, height: 350, fontSize: 240,
  colorStart: "#F66C02", colorEnd: "#C91F8A",
});
slide.addImage({ data: heroNum, x: 3.5, y: 1.0, w: 6.0, h: 2.5 });
```

Tips:
- Use `width`/`height` large enough so text doesn't clip (test with your longest string)
- Set slide `w`/`h` proportionally to the SVG aspect ratio to avoid stretching
- For multi-line gradient text, use multiple `<text>` elements with different `y` offsets
...
8. **ROUNDED_RECTANGLE is the default for cards** - use `line` property for accent colors instead of overlay bars.
   ```javascript
   // ❌ WRONG: Overlay accent bar on rounded corners
   slide.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 1, y: 1, w: 3, h: 1.5, rectRadius: 0.1, fill: { color: "161E2D" } });
   slide.addShape(pres.shapes.RECTANGLE, { x: 1, y: 1, w: 0.08, h: 1.5, fill: { color: "0891B2" } });

   // ✅ CORRECT: Use line border for accent on rounded card
   slide.addShape(pres.shapes.ROUNDED_RECTANGLE, {
     x: 1, y: 1, w: 3, h: 1.5, rectRadius: 0.1,
     fill: { color: "161E2D" },
     line: { color: "0891B2", width: 1.5 }
   });
   ```
   Only use plain `RECTANGLE` for full-slide overlays, table cells, and edge gradient bars.
